...
{
    void* data = lua_newuserdata(L, sizeof(Lua_Text));
    new (data) Lua_Text(string, *font, charSize);
    luaL_getmetatable(L, LUA_TEXT_METATABLENAME);
    lua_setmetatable(L, -2);
}
...
int TextExporter::__new(lua_State *L)
{
    ...
    Lua_Font* font = static_cast<Lua_Font*>( luaL_checkudata(L, 2, LUA_FONT_METATABLENAME) );
...
int TextExporter::__index(lua_State* L)
{
    ...
    {
        lua_pushinteger(L, Lua_Text::Style::Regular);
        return 1;
    }
    else if(indexStr == "Style_Bold")
    {
        lua_pushinteger(L, Lua_Text::Style::Bold);
        return 1;
    }
    else if(indexStr == "Style_Italic")
    {
        lua_pushinteger(L, Lua_Text::Style::Italic);
        return 1;
    }
    else if(indexStr == "Style_Underlined")
    {
        lua_pushinteger(L, Lua_Text::Style::Underlined);
        return 1;
    }
    else if(indexStr == "Style_StrikeThrough")
    {
        lua_pushinteger(L, Lua_Text::Style::StrikeThrough);
        return 1;
    }
...
{
    Lua_Text* textPtr = static_cast<Lua_Text*>( luaL_checkudata(L, 1, LUA_TEXT_METATABLENAME) );
    Lua_Font* fontPtr = static_cast<Lua_Font*>( luaL_checkudata(L, 2, LUA_FONT_METATABLENAME) );
...
int TextExporter::getPosition(lua_State* L)
{
    Lua_Text* textPtr = static_cast<Lua_Text*>( luaL_checkudata(L, 1, LUA_TEXT_METATABLENAME) );
...
LuaExporter TextExporter::toLuaExporter()
{
    LuaExporter exporter(
        ...
        __new,
        {
            ...
            {"getPosition", TextExporter::getPosition},
    ...
    exporter.addInteger("Style_Regular", Lua_Text::Style::Regular);
    exporter.addInteger("Style_Bold", Lua_Text::Style::Bold);
    exporter.addInteger("Style_Italic", Lua_Text::Style::Italic);
    exporter.addInteger("Style_Underlined", Lua_Text::Style::Underlined);
    exporter.addInteger("Style_StrikeThrough", Lua_Text::Style::StrikeThrough);
