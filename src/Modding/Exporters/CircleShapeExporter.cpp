...
void CircleShapeExporter::createCircleShape(lua_State* L, float radius, std::size_t pointCount)
{
    ...
    new (data) Lua_CircleShape(radius, pointCount);
...
{
    Lua_CircleShape* circleShapePtr = static_cast<Lua_CircleShape*>( luaL_checkudata(L, 1, LUA_CIRCLESHAPE_METATABLENAME) );
...
{
    Lua_CircleShape* circleShapePtr = static_cast<Lua_CircleShape*>( luaL_checkudata(L, 1, LUA_CIRCLESHAPE_METATABLENAME) );
    LuaHelper::push(L, {(void*)circleShapePtr->getTexture(), LUA_TEXTURE_METATABLENAME});
...
int CircleShapeExporter::getPosition(lua_State* L)
{
    Lua_CircleShape* circleShapePtr = static_cast<Lua_CircleShape*>( luaL_checkudata(L, 1, LUA_CIRCLESHAPE_METATABLENAME) );
...
LuaExporter CircleShapeExporter::toLuaExporter()
{
    LuaExporter exporter(
        ...
        __new,
        {
            ...
            {"getPosition", CircleShapeExporter::getPosition},
