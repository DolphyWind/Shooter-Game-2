...
void SoundBufferExporter::createSoundBuffer(lua_State* L, const Exported_SoundBuffer& soundBuffer)
{
    void* data = lua_newuserdata(L, sizeof(Exported_SoundBuffer));
    new (data) Exported_SoundBuffer(soundBuffer);
...
int SoundBufferExporter::__new(lua_State *L)
{
    ...
    createSoundBuffer(L, Exported_SoundBuffer());
...
{
    Exported_SoundBuffer* soundBufferPtr = static_cast<Exported_SoundBuffer*>( LuaHelper::checkudata_WithError(L, 1, LUA_SOUNDBUFFER_METATABLENAME) );
    soundBufferPtr->~Exported_SoundBuffer();
    return 0;
}
